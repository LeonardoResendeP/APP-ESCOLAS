...
@page { 
  size: A4; 
  margin: 6mm;  /* Reduzido de 10mm para 6mm */
}
...
body { font-family: -apple-system, Segoe UI, Roboto, Arial, sans-serif; color: #111827; font-size: 10.5px;line-height: 1.2; }
...
.subtitle {
  font-size: 14px;
  color: #4b5563; /* Um cinza-azulado suave, consistente com a classe .sub existente */
  font-weight: normal;
  margin-top: 2px;
  margin-bottom: 2px;
  line-height: 1.4;
...
.sub { color: #4b5563; }
/* Nova classe para layout de duas colunas */
.two-column-layout {
  display: grid;
...
.kpi { flex:1 1 0; border:1px solid #E5E7EB; border-radius:10px; padding:6px; }
...
.card.ai p { 
  margin: 2px 0;      /* Reduzido de 4px */
  line-height: 1.2;   /* Reduzido de 1.28 */
}
...
header_div <- div(class = "header",
  div(class = "h-left",
    h1("Demonstrativo Plataforma Explora"
    ),
    div(class = "subtitle",h4("Pesquisas, métricas e tendências educacionais")
    ),
    div(class = "sub",
...
left_col <- div(class = "conc-card",
  ...
  div(class = "conc-note", style="margin-top:6px;",
      tags$small("Você pode personalizar os concorrentes na plataforma, escaneia o QR ao lado para desbloquear todas as funcionalidades.")
  )
)
...
### Matrículas por segmento (2023→2024)

```{r}
df <- params$tabelas$matriculas
if (is.data.frame(df) && nrow(df) > 0) {
  # garante nomes
  colnames(df) <- c("Segmento","2023","2024","Variação Escola.","Variação Concorrentes.","Variação Municipio.")
  ...
  df |>
    ...
    kableExtra::add_header_above(c(" " = 1, "Quantidade por ano:" = 2, "Variações (%%)" = 3))
} else {
